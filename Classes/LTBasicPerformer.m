...
@implementation LTBasicPerformer
...
{
	return [LTManagedObjectContext executeFetchRequest:[fetchRequests valueForKey:key] error:nil];
}
...
{
	NSManagedObject *object = [NSEntityDescription insertNewObjectForEntityForName:entity inManagedObjectContext:LTManagedObjectContext];
...
- (void)removeAllObjectsForEntity:(NSString *)entity
{
	...
	for (id item in array) {
		[LTManagedObjectContext deleteObject:item];
	}
}
...
- (NSString *)genererateTemporaryPath
{
	...
	do {
		sequenceNumber++;
		temporaryPath = [NSString stringWithFormat:@"%ld-%ld-%ld.%@", [[NSProcessInfo processInfo] processIdentifier], (NSInteger)[NSDate timeIntervalSinceReferenceDate], sequenceNumber, @"Literate"];
