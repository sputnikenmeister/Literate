...
@implementation LTProject (ToolbarController)
...
//		NSToolbarItem *toolbarItem = [[NSToolbarItem alloc] initWithItemIdentifier:itemIdentifier];
//		if ([[LTDefaults valueForKey:@"LineWrapNewDocuments"] boolValue] == NO) {
...
//{
//	// Function and Live Find are validated in -[LTProject documentsListHasUpdated] so that they don't need to be updated all the time as they would have been here 
//	
//	BOOL enableItem = YES;
//	Log(toolbarItem);
...
- (void)open:(id)sender
{
	[[LTFileMenuController sharedInstance] openAction:nil];
}


- (void)new:(id)sender
{
	[[LTFileMenuController sharedInstance] newAction:nil];
}


- (void)close:(id)sender
{
	[self checkIfDocumentIsUnsaved:LTCurrentDocument keepOpen:NO];
}


- (void)print:(id)sender
{
	[[LTFileMenuController sharedInstance] printAction:nil];
}
...
{
	[[LTToolsMenuController sharedInstance] previewAction:nil];
}
...
- (IBAction)functionToolbarItemAction:(id)sender
{
	...
	[LTBasic removeAllItemsFromMenu:functionMenu];
	[LTInterface insertAllFunctionsIntoMenu:functionMenu];
...
{
	[LTCurrentWindow makeKeyAndOrderFront:nil];
...
- (void)removeLiveFindSession
{
	...
	[LTCurrentWindow makeFirstResponder:liveFindSearchField];
	...
	[LTCurrentWindow makeFirstResponder:firstResponder];
}
...
- (void)menuNeedsUpdate:(NSMenu *)menu
{
	if ([[self projectWindowToolbar] displayMode] == NSToolbarDisplayModeLabelOnly) { // This is only needed if the toolbar is text only
		...
		[LTInterface insertAllFunctionsIntoMenu:menu];
...
//{
//	[[LTViewMenuController sharedInstance] splitWindowAction:nil];
//}
...
//{
//	[[LTViewMenuController sharedInstance] lineWrapTextAction:nil];
//}
//
//
//- (void)updateLineWrapToolbarItem
//{
//	if ([[LTApplicationDelegate sharedInstance] hasFinishedLaunching] == YES) {
//		if ([[LTCurrentDocument valueForKey:@"isLineWrapped"] boolValue] == NO) {
...
- (void)extraToolbarValidation
{
	if ([self areThereAnyDocuments] == YES) {
		...
		if (liveFindToolbarItem != nil && [[LTApplicationDelegate sharedInstance] hasFinishedLaunching] == YES) {
