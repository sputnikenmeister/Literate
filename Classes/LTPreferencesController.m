...
+ (LTPreferencesController *)sharedInstance
{ 
...
{	
	...
	[dictionary setValue:[NSNumber numberWithInteger:LTOpenSaveRemember] forKey:@"OpenMatrix"];
	[dictionary setValue:[NSNumber numberWithInteger:LTOpenSaveAlways] forKey:@"SaveMatrix"];
	...
	[dictionary setValue:[NSNumber numberWithBool:YES] forKey:@"AssignDocumentToLiterateWhenSaving"];
	...
	if ([(NSString *)context isEqualToString:@"StatusBarChanged"]) {
		...
		for (id item in array) {
			if ([[item valueForKey:@"isNewDocument"] boolValue] == NO) {
				[LTVarious setLastSavedDateForDocument:item date:[[item valueForKey:@"fileAttributes"] fileModificationDate]];
			}
		}		
		[LTInterface updateStatusBar];
		
	} else if ([(NSString *)context isEqualToString:@"ShowFullPathInWindowTitleChanged"]) {
		NSArray *projectsArray = [[LTProjectsController sharedDocumentController] documents];
		for (id project in projectsArray) {
			NSArray *documentsArray = [LTBasic fetchAll:@"Document"];
		...
		if ([[LTCommandsController sharedInstance] commandsWindow] != nil) {
			[[[LTCommandsController sharedInstance] commandCollectionsTableView] reloadData];
			[[[LTCommandsController sharedInstance] commandsTableView] reloadData];
		}
		
		if ([[LTSnippetsController sharedInstance] snippetsWindow] != nil) {
			[[[LTSnippetsController sharedInstance] snippetCollectionsTableView] reloadData];
			[[[LTSnippetsController sharedInstance] snippetsTableView] reloadData];
		}
		if ([[LTAdvancedFindController sharedInstance] advancedFindWindow] != nil) {
			[[[LTAdvancedFindController sharedInstance] findResultsOutlineView] reloadData];
		}	
...
{	
	if (preferencesWindow == nil) {
		[NSBundle loadNibNamed:@"LTPreferences.nib" owner:self];
		...
		[preferencesWindow setTitle:NSLocalizedStringFromTable(@"Preferences - Literate", @"Localizable3", @"Preferences - Literate")];
	}
	
	if ([preferencesToolbar selectedItemIdentifier] == nil) {
		if (generalView == nil) {
			[NSBundle loadNibNamed:@"LTPreferencesGeneral.nib" owner:self];
		}
		[preferencesToolbar setSelectedItemIdentifier:@"GeneralPreferencesToolbarItem"];
		if ([LTDefaults valueForKey:@"PreferencesGeneralViewSavedFrame"] == nil) {
			...
			NSRect temporaryRect = NSRectFromString([LTDefaults valueForKey:@"PreferencesGeneralViewSavedFrame"]);
...
- (NSToolbarItem *)toolbar:(NSToolbar *)toolbar itemForItemIdentifier:(NSString *)itemIdentifier willBeInsertedIntoToolbar:(BOOL)willBeInserted
{
    if ([itemIdentifier isEqualToString:@"GeneralPreferencesToolbarItem"]) {
		...
		NSImage *appearanceImage = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"LTAppearanceIcon" ofType:@"pdf" inDirectory:@"Preferences Icons"]];
		...
		NSImage *openSaveImage = [[NSImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"LTOpenSaveIcon" ofType:@"pdf" inDirectory:@"Preferences Icons"]];
...
{	
	...
	if ([identifier isEqualToString:@"GeneralPreferencesToolbarItem"]) {
		...
		[[preferencesWindow contentView] addSubview:generalView];
		[LTInterface changeViewWithAnimationForWindow:preferencesWindow oldView:currentView newView:generalView newRect:[self getRectForView:generalView]];
		...
		if (appearanceView == nil) {
			[NSBundle loadNibNamed:@"LTPreferencesAppearance.nib" owner:self];			
		}
		...
		[LTInterface changeViewWithAnimationForWindow:preferencesWindow oldView:currentView newView:appearanceView newRect:[self getRectForView:appearanceView]];
		[currentView removeFromSuperviewWithoutNeedingDisplay];
		currentView = appearanceView;
		
	} else if ([identifier isEqualToString:@"OpenSavePreferencesToolbarItem"]) {
		if (openSaveView == nil) {
			[NSBundle loadNibNamed:@"LTPreferencesOpenSave.nib" owner:self];
		}
		if (currentView == openSaveView) {
			return;
		}
		[[preferencesWindow contentView] addSubview:openSaveView];
		[LTInterface changeViewWithAnimationForWindow:preferencesWindow oldView:currentView newView:openSaveView newRect:[self getRectForView:openSaveView]];
		[currentView removeFromSuperviewWithoutNeedingDisplay];
		currentView = openSaveView;
		
	} else if ([identifier isEqualToString:@"AdvancedPreferencesToolbarItem"]) {
		if (advancedView == nil) {
			[NSBundle loadNibNamed:@"LTPreferencesAdvanced.nib" owner:self];
		}
		if (currentView == advancedView) {
			return;
		}
		if (hasPreparedAdvancedInterface == NO) {
			[LTBasic removeAllItemsFromMenu:[encodingsPopUp menu]];
			
			[self buildEncodingsMenu];
			
			// Build syntax definitions menu		
			[LTBasic removeAllItemsFromMenu:[syntaxColouringPopUp menu]];

			NSEnumerator *enumerator = [[LTBasic fetchAll:@"SyntaxDefinitionSortKeySortOrder"] reverseObjectEnumerator];
			NSMenuItem *menuItem;
		...
		[LTInterface changeViewWithAnimationForWindow:preferencesWindow oldView:currentView newView:advancedView newRect:[self getRectForView:advancedView]];
		[currentView removeFromSuperviewWithoutNeedingDisplay];
...
- (void)changeFont:(id)sender
{
	...
	[LTDefaults setValue:[NSArchiver archivedDataWithRootObject:panelFont] forKey:@"TextFont"];
}
...
- (void)openPanelDidEnd:(NSOpenPanel *)sheet returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSOKButton) {						
		[LTDefaults setValue:[[sheet filename] stringByAbbreviatingWithTildeInPath] forKey:@"OpenAlwaysUseTextField"];
	}
}
