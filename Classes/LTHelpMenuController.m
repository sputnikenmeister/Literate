...
- (void)installCommandLineUtiltiySheetDidEnd:(NSWindow *)sheet returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSAlertDefaultReturn) {
		[[LTAuthenticationController sharedInstance] installCommandLineUtility];
	} else if (returnCode == NSAlertAlternateReturn) {
		NSFileManager *fileManager = [NSFileManager defaultManager];
		NSWorkspace *workspace = [NSWorkspace sharedWorkspace];
		NSString *pathToFolder = [[NSHomeDirectory() stringByAppendingPathComponent:@"Desktop"]
        stringByAppendingPathComponent:@"Literate command-line utility"];
...
{
	[[NSWorkspace sharedWorkspace] openFile:[[NSBundle mainBundle] pathForResource:@"Literate-Manual" ofType:@"pdf"]];
}
