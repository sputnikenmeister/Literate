...
- (id)initWithDocument:(id)theDocument
{
	if (self = [super init]) {
		...
		attributes = [[NSDictionary alloc] initWithObjectsAndKeys:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextFont"]], NSFontAttributeName, nil];
...
- (void)updateLineNumbersForClipView:(NSClipView *)clipView checkWidth:(BOOL)checkWidth recolour:(BOOL)recolour
{
	...
	if (scrollView == [document valueForKey:@"firstTextScrollView"]) {
		...
		addToScrollPoint = [[LTCurrentProject secondContentViewNavigationBar] bounds].size.height;
