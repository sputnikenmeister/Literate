...
@implementation LTAuthenticationController
...
- (void)performAuthenticatedSaveOfDocument:(id)document data:(NSData *)data path:(NSString *)path fromSaveAs:(BOOL)fromSaveAs aCopy:(BOOL)aCopy
{
	...
	if (status != 0) {
		...
		[LTVarious standardAlertSheetWithTitle:title message:NSLocalizedStringFromTable(@"Please check if the file is locked, on a media that is unwritable or if you can save it in another location", @"Localizable3", @"Please check if the file is locked, on a media that is unwritable or if you can save it in another location") window:LTCurrentWindow];
	} else {
		if (!aCopy) {
			[LTOpenSave updateAfterSaveForDocument:document path:path];
		}
	}
}
...
- (void)installCommandLineUtility
{
	...
	if (status != 0) {
		[LTVarious standardAlertSheetWithTitle:NSLocalizedString(@"There was a unknown error when trying to install the command-line utility", @"Indicate that there was a unknown error when trying to install the command-linbe utility in Unknown-error-when-installing-comman-line-utility sheet") message:@"" window:LTCurrentWindow];
	}	
}
