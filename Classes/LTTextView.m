...
- (void)setDefaults
{
	...
	[self setContinuousSpellCheckingEnabled:[[LTDefaults valueForKey:@"AutoSpellCheck"] boolValue]];
	[self setGrammarCheckingEnabled:[[LTDefaults valueForKey:@"AutoGrammarCheck"] boolValue]];
	
	[self setSmartInsertDeleteEnabled:[[LTDefaults valueForKey:@"SmartInsertDelete"] boolValue]];
	[self setAutomaticLinkDetectionEnabled:[[LTDefaults valueForKey:@"AutomaticLinkDetection"] boolValue]];
	[self setAutomaticQuoteSubstitutionEnabled:[[LTDefaults valueForKey:@"AutomaticQuoteSubstitution"] boolValue]];
	
	[self setFont:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextFont"]]];
	[self setTextColor:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextColourWell"]]];
	[self setInsertionPointColor:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextColourWell"]]];
	[self setBackgroundColor:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"BackgroundColourWell"]]];
	...
	lineHeight = [[[self textContainer] layoutManager] defaultLineHeightForFont:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextFont"]]];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	if ([(NSString *)context isEqualToString:@"TextFontChanged"]) {
		[self setFont:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextFont"]]];
		lineHeight = [[[self textContainer] layoutManager] defaultLineHeightForFont:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextFont"]]];
		[[LTCurrentDocument valueForKey:@"lineNumbers"] updateLineNumbersForClipView:[[self enclosingScrollView] contentView] checkWidth:NO recolour:YES];
		[self setPageGuideValues];
...
- (void)insertNewline:(id)sender
{
	...
	if ([[LTDefaults valueForKey:@"IndentNewLinesAutomatically"] boolValue] == YES) {
		...
		if ([[LTDefaults valueForKey:@"AutomaticallyIndentBraces"] boolValue] == YES) {
...
{	
	...
	if (shouldShiftText) {
		[[LTTextMenuController sharedInstance] shiftRightAction:nil];
	} else if ([[LTDefaults valueForKey:@"IndentWithSpaces"] boolValue] == YES) {
		NSMutableString *spacesString = [NSMutableString string];
		NSInteger numberOfSpacesPerTab = [[LTDefaults valueForKey:@"TabWidth"] integerValue];
...
- (void)setTabWidth
{
	...
	NSInteger numberOfSpaces = [[LTDefaults valueForKey:@"TabWidth"] integerValue];
	while (numberOfSpaces--) {
		[sizeString appendString:@" "];
	}
	NSDictionary *sizeAttribute = [[NSDictionary alloc] initWithObjectsAndKeys:[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextFont"]], NSFontAttributeName, nil];
...
- (void)setPageGuideValues
{
	...
	pageGuideX = (sizeOfCharacter * ([[LTDefaults valueForKey:@"ShowPageGuideAtColumn"] integerValue] + 1)) - 1.5; // -1.5 to put it between the two characters and draw only on one pixel and not two (as the system draws it in a special way), and that's also why the width above is set to zero 
	
	NSColor *color = [NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextColourWell"]];
	...
	showPageGuide = [[LTDefaults valueForKey:@"ShowPageGuide"] boolValue];
...
{
	if ([aString isEqualToString:@"}"] && [[LTDefaults valueForKey:@"IndentNewLinesAutomatically"] boolValue] == YES && [[LTDefaults valueForKey:@"AutomaticallyIndentBraces"] boolValue] == YES) {
	...
	} else if ([aString isEqualToString:@"("] && [[LTDefaults valueForKey:@"AutoInsertAClosingParenthesis"] boolValue] == YES) {
		[super insertText:aString];
	...
	} else if ([aString isEqualToString:@"{"] && [[LTDefaults valueForKey:@"AutoInsertAClosingBrace"] boolValue] == YES) {
		[super insertText:aString];
		NSRange selectedRange = [self selectedRange];
		if ([self shouldChangeTextInRange:selectedRange replacementString:@"}"]) {
			[self replaceCharactersInRange:selectedRange withString:@"}"];
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
	...
	for (id collection in collectionEnumerator) {
		...
		for (id snippet in array) {
			...
			[subMenuItem setTarget:[LTToolsMenuController sharedInstance]];			
...
{
	NSColor *textColour = [[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextColourWell"]] colorUsingColorSpaceName:NSCalibratedWhiteColorSpace];
	...
	if (textColour != nil && [textColour whiteComponent] == 0.0 && [textColour alphaComponent] == 1.0) { // Keep the original cursor if it's black
		...
		[[NSUnarchiver unarchiveObjectWithData:[LTDefaults valueForKey:@"TextColourWell"]] set];
